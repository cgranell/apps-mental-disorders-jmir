---
title: 'Analysis and visualisations for "Smartphone apps for the treatment of mental disorders: a systematic review" (Second Review)'
...
library(kableExtra)
library(scales)
# library(DT)       # Alternative outpur of table.
library(knitr)
...
# Alternative: Same output with DT package
# data_tbl_assesstype %>%
#     arrange(val_type) %>%
#     select(`Assessment type` = val_type,
#            `N (%)` = n_lbl,
...
plot_tempdist_stakedbarchart <- 
    data_plot_tempdist %>%
        ...
        labs(#title="Count of papers per year colored by assessment type", 
             x="Year", 
             y="# of papers - Percentage of papers [%]") +
...
cowplot::ggsave2(plot = plot_tempdist_stakedbarchart, filename = plot_tempdist_file_name, device = "png", 
                 path = here::here("figs"), scale = 1, width = 18, height = 25, units = "cm", dpi = 600)
...
plot_barchart <- 
    data_plot_barchart %>%
      ggplot(aes(x=md_desc, y=proportion, fill=val_type)) + 
        ...
        guides(fill=guide_legend(title="Assessment Type", nrow=1)) + # modify legend title
        theme_minimal()  +
        theme(legend.title = element_text(size=11),
...
### Table 2: Apps adn studies grouped by mental disorder. NA = app name not available/not mentioned. Apps covered in multiple publications are put in italics.
...
Out of N=`r n_papers`, there are `r nrow(data_all_apps)` studies with app names. However, there are `r nrow(data_apps_in_papers)` unique apps which follow a long tail distribution, since `r nrow(data_apps_in_papers) - nrow(apps_in_multiple_papers)` appear only once, while `r nrow(apps_in_multiple_papers)` apps appear in two or more studies (an app in two studies is the most frequent case (`r apps_in_2_papers`); an app in 3 or more studies occurs in `r nrow(apps_in_multiple_papers) - apps_in_2_papers` cases. In summary, the percentage of apps that have been studied (reused) in multiple articles is `r nrow(apps_in_multiple_papers)` / `r nrow(data_all_papers)` **(`r percent(nrow(apps_in_multiple_papers)/nrow(data_all_apps))`)**. 
