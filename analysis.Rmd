...
plot_tempdist_stakedbarchart <- 
    data_plot_tempdist %>%
        ...
        geom_text(aes(label=number_cases), size=2.7, position=position_stack(vjust = 0.5)) +
...
plot_tempdist_areachart <- 
    data_plot_tempdist %>%
        ...
        scale_fill_manual(values = pal) +
        scale_y_continuous(expand=c(0,0), labels=scales::percent_format(), breaks=seq(0, 1.00, by=0.10), limits=c(0, 1.00)) +
        theme_minimal() +
        # Change the line type and color of axis lines
        theme(axis.line = element_line(colour = "darkgray", size = 0.5, linetype = "solid")) +
...
cowplot::ggsave2(plot = plot_tempdist, filename = plot_tempdist_file_name, device = "png", 
                 path = here::here("figs"), scale = 1, width = 20, height = 28, units = "cm", dpi = 600)
