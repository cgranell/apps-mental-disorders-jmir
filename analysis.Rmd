...
## MODIFICATION TO CONVERT PIECHART TO TABLE
# library(ggpmisc)
data_table <-
    data_plot_barchart %>%
    group_by(val_type) %>%
    summarise(total_asstype = sum(number_cases)) %>%
    mutate(lbl = round(total_asstype/n_papers*100,1), 
           `n (%)`= paste0(total_asstype, " (", lbl, ")")) %>% 
    select(-total_asstype, -lbl, `Assessment types`=val_type)

data_table[1] <- c("No assessment", "Usability/UX", "Effect + Usability/UX", "Effect")

data_table <- as_data_frame(data_table)
## END MODIFICATION TO CONVERT PIECHART TO TABLE
